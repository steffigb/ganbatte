...
> **Last updated:** 2026-08-30 (paginated sync pull + force full resync; lesson setup type filter)  
...
- [x] **Search: filters work on the whole dataset, not just query matches** (2026-08-11) — `/search`'s facet filters (type, level, skill, part of speech, verb type, transitivity, mastery, weak-only) previously only ran once a non-empty query narrowed the pool first — `searchLocal()` and `searchAll()` both short-circuited to empty results on an empty query, and `useSearch()`'s `canSearch` gate required a non-empty debounced query too, so e.g. "show all mastered kanji" needed typing something first. All three short-circuits removed; `searchLocal()` now only skips the free-text match itself when the query is empty (`hasQuery` guard, since `textIncludesQuery`/`textStartsWithQuery` return `false` for an empty query by design), letting every other filter run against the full dataset and sort alphabetically. `useSearch()`'s `canSearch` is now just "user is logged in." Removed the now-dead `hasQuery` prop/placeholder ("Start typing to search…") threaded through `SearchView` → `SearchResultsView`. `findSimilarItemsLocal()`/`findSimilarItems()` (duplicate-item suggestions while typing on add-item forms) intentionally left query-gated — different use case
- [x] **Browse page: Reference filter** (2026-08-11) — `/learn/:skill` gained a fourth filter dropdown, **Reference**, alongside Level/Topic/Source, filtering by the free-text `reference` field on an item's source link (e.g. chapter/page within a book). `RelationFilter` extended to `{ topicId, sourceId, sourceRef }` and `deriveRelationOptions()`/`matchesRelationFilter()` (`features/items/itemRelationFilters.ts`) updated accordingly — shared with lesson setup's filter row, though `LessonSetup` doesn't render a Reference control yet, so it stays `'all'` there. Same conditional-render-if-options-exist pattern as Topic/Source
- [x] **Bulk import: multi-source rows + opt-in source replacement** (2026-08-11) — re-importing a CSV of items that already exist (`duplicateAction: 'update'`) updated plain fields (meaning, onyomi, notes, ...) correctly, but `source`/`source_ref` were handled by a separate `attachSource()` step that only ever *added* a link, never removed one — so there was no way to change an item's source via CSV, and per-item sources set by hand could get silently duplicated. Fixed in two parts: (1) `source`/`source_ref` cells now accept `;`-separated lists, positionally paired (`Genki 1; Nihongo So-matome N5` + `chapter 3; lesson 4` → two distinct `ItemSource` links, each independently filterable via the Source/Reference filters above) — `parseSourceList()` (`normalizeField.ts`), `ParsedImportRow.sources: { label, reference }[]` (was `sourceLabel`/`sourceRef` singular). (2) Replacing vs. adding sources is now its own **"Replace existing source instead of adding to it"** checkbox (`ImportOptions.replaceSource`, default off), independent of the `duplicateAction` dropdown — initially this was tied to `duplicateAction === 'update'`, but that meant routine text-field fixes silently overwrote hand-set sources whenever the CSV happened to carry a value in that column, which is a materially different, more destructive operation than "update this item's fields." `attachSources()` (renamed from `attachSource`, `executeImport.ts`) diffs the row's resolved source ids against the item's current links and deletes the ones missing from the row only when the checkbox is on
...
| 2026-08-08 | **Lessons session restart bug diagnosed and fixed** — user noticed that stepping away from a Lessons session (e.g. to look up a word) and coming back always restarted the session at the first item. Root cause: Lessons never persists anything to `user_progress` per item, only on Finish (`completeLessons()`), and `currentIndex` was plain in-memory `useState` inside `useLessonSession` — navigating away unmounts `/learn/lessons/:group`, which throws that state away, and remounting reloads the same "not yet learned" batch from index 0. (The Study/review queue has the identical reset-on-remount pattern in `useReviewSession`, but it's far less disruptive there since each grade is persisted immediately as you go.) Fixed by persisting the current item's id to `sessionStorage` per group and restoring it on mount (falls back to index 0 if the saved id isn't found in a freshly-loaded batch, e.g. after Finish or if the underlying item list changed) — deliberately `sessionStorage`, not `localStorage`, since the goal is surviving a short in-app detour within the same browser session, not resuming a lesson left half-finished days ago |
